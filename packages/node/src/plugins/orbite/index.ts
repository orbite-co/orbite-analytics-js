...
function normalizeEvent(ctx: Context) {
  ctx.updateEvent('context.library.name', '@orbite/analytics-node')
...
type OrbiteNodePlugin = Plugin & Required<Pick<Plugin, DefinedPluginFields>>

export type ConfigureNodePluginProps = PublisherProps

export function createNodePlugin(publisher: Publisher): OrbiteNodePlugin {
