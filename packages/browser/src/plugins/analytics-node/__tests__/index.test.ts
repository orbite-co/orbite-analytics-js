  ...
  describe('AJS', () => {
    test('fireEvent instantiates the right event types', async () => {
      ...
      expect(fetcher).toHaveBeenCalledWith(
        'https://cdp.orbite.co/v1/track',
        expect.anything()
      )

      await ajs.identify('identify')
      expect(fetcher).toHaveBeenCalledWith(
        'https://cdp.orbite.co/v1/identify',
        expect.anything()
      )

      await ajs.page('page')
      expect(fetcher).toHaveBeenCalledWith(
        'https://cdp.orbite.co/v1/page',
        expect.anything()
      )

      await ajs.group('group')
      expect(fetcher).toHaveBeenCalledWith(
        'https://cdp.orbite.co/v1/group',
        expect.anything()
      )

      await ajs.alias('alias')
      expect(fetcher).toHaveBeenCalledWith(
        'https://cdp.orbite.co/v1/alias',
        expect.anything()
      )

      await ajs.screen('screen')
      expect(fetcher).toHaveBeenCalledWith(
        'https://cdp.orbite.co/v1/screen',
        expect.anything()
      )
    })
  })
